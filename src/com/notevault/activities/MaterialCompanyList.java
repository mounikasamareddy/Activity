...
public class MaterialCompanyList extends Activity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		MaterialCompanyTask eqName = new MaterialCompanyTask();
		...
		addImageLayout.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if(singleton.isOnline()) {
				Intent intent = new Intent(MaterialCompanyList.this,AddMaterialCompany.class);
	...
	private class MaterialCompanyTask extends AsyncTask<Void, Void, String> {
		...
		protected void onPostExecute(String response) {
            if(response != null){
                try {
                    JSONObject materialCompanyObj = new JSONObject(response);
                    JSONArray materialCompanyArray = new JSONArray(materialCompanyObj.getString("CompanyNames"));
                    company.clear();
    ...
    public  void setAdapter(){
        ...
        ArrayAdapter<String> ad = new ArrayAdapter<String>(MaterialCompanyList.this, android.R.layout.simple_list_item_1, company);
        nameListView.setAdapter(ad);
        nameListView.setOnItemClickListener(new OnItemClickListener() {

            @Override
            public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
                singleton.setSelectedMaterialCompany((String) (nameListView.getItemAtPosition(arg2)));
                onBackPressed();
            }
        });
    }
    ...
    public  void readGlossaryFromDB(){
    	company.clear();
    	Log.d("data", "--->" + singleton.getCCID());
		List<ECmpany> data = dbAdapter.getAllCCompanyRecords(singleton.getCCID());

		for (ECmpany val : data) {

			company.add(val.getECmpany());

		}
		Collections.sort(company);
		for (int i = 0; i < company.size(); i++) {
			Log.d("data", "---->" + company.get(i));
		}
        dbAdapter.Close();
        setAdapter();
    }

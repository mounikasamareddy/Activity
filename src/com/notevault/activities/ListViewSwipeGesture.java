...
public class ListViewSwipeGesture implements View.OnTouchListener {
	...
	String RangeOneColor="#f49800";   //"#FFD060"
	...
	private void GetResourcesValues() {
		...
		HalfDrawable        =   activity.getResources().getDrawable( R.drawable.copy_to_yesterday );
		FullDrawable        =   activity.getResources().getDrawable(R.drawable.copy_to_today);
	}
...
//COPIED FROM TOUCH DOWN STARTS HERE
/*				if (mPaused) {
					return false;
				}
				
			if(Math.abs(deltaX) > (mListView.getWidth()*2/5)){
				Rect rect               =   new Rect();
				int childCount          =   mListView.getChildCount();
				int[] listViewCoords    = new int[2];
				mListView.getLocationOnScreen(listViewCoords);
...
//COPIED FROM TOUCH DOWN ENDS HERE
